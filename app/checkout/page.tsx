...
export default function CheckoutPage() {
  ...
  const handleWhatsAppOrder = async () => {
    let customerEmail = (formData.email || session?.user?.email || '').trim().toLowerCase();
    if (!customerEmail) {
      customerEmail = window.prompt('Enter your email so we can save and track this order:')?.trim().toLowerCase() || '';
    }
    if (!/^\S+@\S+\.\S+$/.test(customerEmail)) {
      toast.error('A valid email is needed to save this order.');
      return;
    }
    ...
    try {
      ...
      const orderReference = response.data.order.order_number;
...
Please confirm this order. Thank you!`;
      ...
      clearCart();
      toast.success(`Order ${orderReference} saved. Opening WhatsApp...`);
    } catch (error: any) {
      whatsappWindow?.close();
      toast.error(error.response?.data?.error || 'Could not save your order.');
    } finally {
      setLoading(false);
    }
  };
          ...
          <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded p-6">
            ...
            <div className="mt-6 text-center">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 text-green-600 hover:text-green-700 font-medium text-sm disabled:cursor-not-allowed disabled:opacity-50"
              >
