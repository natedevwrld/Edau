    ...
    <DashboardLayout role="admin">
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_420px]">
        ...
        <form onSubmit={submitOrder} className="rounded-xl bg-white p-6 shadow">
          <h2 className="mb-4 flex items-center justify-between text-xl font-semibold"><span className="flex items-center gap-2"><ShoppingCart className="h-5 w-5" /> Current order</span><span className="text-sm font-normal text-gray-500">{cart.length} item{cart.length === 1 ? '' : 's'}</span></h2>
          <div className="mb-5 space-y-3">{cart.length === 0 ? <div className="rounded-xl border border-dashed border-gray-300 bg-gray-50 p-8 text-center text-sm text-gray-500">Select products to begin.</div> : cart.map((item) => <div key={item.id} className="rounded-xl border border-gray-200 bg-white p-3 transition hover:border-primary-200 hover:shadow-sm"><div className="flex gap-3"><div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-lg bg-gray-100">{item.images?.[0] && <Image src={item.images[0]} alt={item.name} fill className="object-cover" sizes="64px" />}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-gray-900">{item.name}</p><p className="mt-1 text-xs text-gray-500">{formatPrice(item.price)} / {item.unit_type || 'piece'}</p><p className="mt-1 text-sm font-bold text-primary-700">{formatPrice(item.price * item.orderedQuantity)}</p></div><button type="button" onClick={() => setCart((items) => items.filter((line) => line.id !== item.id))} className="h-fit rounded-lg p-1.5 text-red-600 hover:bg-red-50" aria-label={`Remove ${item.name}`}><Trash2 className="h-4 w-4" /></button></div><div className="mt-3 flex items-center justify-between border-t border-gray-100 pt-3"><span className="text-xs font-medium uppercase tracking-wide text-gray-500">Quantity</span><div className="flex items-center gap-2"><button type="button" onClick={() => changeQuantity(item.id, -1)} className="rounded-lg border border-gray-300 p-1.5 hover:bg-gray-100" aria-label={`Remove one ${item.name}`}><Minus className="h-4 w-4" /></button><span className="w-7 text-center text-sm font-semibold">{item.orderedQuantity}</span><button type="button" onClick={() => changeQuantity(item.id, 1)} className="rounded-lg border border-gray-300 p-1.5 hover:bg-gray-100" aria-label={`Add one ${item.name}`}><Plus className="h-4 w-4" /></button></div></div></div>)}</div>
          <div className="mb-5 space-y-2 border-b pb-4"><div className="flex justify-between text-sm text-gray-600"><span>Subtotal</span><span className="font-semibold text-gray-900">{formatPrice(total)}</span></div><div className="flex justify-between text-sm text-gray-600"><span>In-person delivery</span><span className="font-semibold text-green-700">Free</span></div><div className="flex justify-between border-t pt-3 text-lg font-bold text-gray-900"><span>Total</span><span className="text-primary-700">{formatPrice(total)}</span></div></div>
          {lastOrderId && (
            <button
              type="button"
